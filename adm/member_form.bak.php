...
if( $mb['mb_10']=='partner' || $mb['mb_level']=='7' ) $print_type='partner';
else $print_type='hunter';


?>
<style>
.bg-green{background:#DFECEC!important;}
</style>
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="row"><?=$config[cf_use_point] ? '포인트':'' ?></th>
        <td>
        <?
		if($config[cf_use_point]){?>
        <a href="./point_list.php?sfl=mb_id&amp;stx=<?php echo $mb['mb_id'] ?>" target="_blank"><?php echo number_format($mb['mb_point']) ?></a> 점 <? }?></td>
    </tr>
    ...
    <tr>
        <th scope="row"><label for="mb_name">이름(담당자)<strong class="sound_only">필수</strong></label></th>
        <td><input type="text" name="mb_name" value="<?php echo $mb['mb_name'] ?>" id="mb_name" required class="required frm_input" size="15"  maxlength="20">
        ()
    ...
    <?
    if($print_type=='hunter'){?>
     <tr>
        <th scope="row">생년월일</th>
        <td>
            <label for="mb_birth" class="sound_only">생년월일<strong>필수</strong></label>           
            
            <select name='birth_year' class="frm_input required" >
            <?
			$birth=explode("-",$mb['mb_birth']);
			for($i=1930;$i < date("Y")-18;$i++){?>
            <option value='<?=$i?>' <?=$birth[0]==$i?'selected':''?> ><?=$i?></option>
            <? }?>
            </select>년
        	
            <select name='birth_month' class="frm_input  required" >
            <?
			for($i=1;$i <= 12;$i++){?>
            <option value='<?=$i?>' <?=$birth[1]*1==$i?'selected':''?> ><?=sprintf("%02d",$i)?></option>
            <? }?>
            </select>월
        	
            <select name='birth_day' class="frm_input  required" >
            <?
			for($i=1;$i <= 31;$i++){?>
            <option value='<?=$i?>' <?=$birth[2]*1==$i?'selected':''?> ><?=sprintf("%02d",$i)?></option>
            <? }?>
            </select>일        
        
        </td>
        <th scope="row">성별</th>
        <td><input type="radio" name="mb_sex" value="M" id="mb_sex_m" <?php echo $mb['mb_sex']=='M'?'checked':'' ?> />
          <label for="mb_sex_m">남성</label>
          &nbsp;
          <input type="radio" name="mb_sex" value="F" id="mb_sex_f" <?php echo $mb['mb_sex']=='F'?'checked':'' ?> />
          <label for="mb_sex_f">여성</label></td>
    </tr>
    <? }?>
	<?
    if($print_type=='hunter'){?>
     ...
     <tr>
        <th scope="row" class='bg-green' >전문분야</th>
        <td colspan="3">
          <input type="text" name="mb_1" value="<?php echo $mb['mb_1'] ?>" id="mb_5" class="frm_input " size="60"  />
        </td>
    </tr>
     <tr>
       <th scope="row" class='bg-green' >경력기간</th>
       <td colspan="3"><input type="text" name="mb_2" value="<?php echo $mb['mb_2'] ?>" id="mb_2" class="frm_input " size="60"  /></td>
     </tr>    
    <? }?> 
    
     <?
    if($print_type=='partner'){?>
    
     <tr>
        <th scope="row" class='bg-green' >업체명</th>
        <td colspan="3">
          <input type="text" name="mb_5" value="<?php echo $mb['mb_5'] ?>" id="mb_5" class="frm_input " size="60"  />
        </td>
    </tr>
     <tr>
       <th scope="row" class='bg-green' >사업자번호</th>
       <td colspan="3"><input type="text" name="mb_4" value="<?php echo $mb['mb_4'] ?>" id="mb_4" class="frm_input " size="60"  /></td>
     </tr>
     <tr>
       <th scope="row"  class='bg-green' ><label for="mb_homepage">홈페이지</label></th>
       <td colspan="3"><input type="text" name="mb_homepage" value="<?php echo $mb['mb_homepage'] ?>" id="mb_homepage" class="frm_input" maxlength="255" size="60" /></td>
     </tr>
    
    <tr>
        <th scope="row"  class='bg-green' >소속부서</th>
        <td colspan="3">
          <input type="text" name="mb_6" value="<?php echo $mb['mb_6'] ?>" id="mb_5" class="frm_input " size="60"  />
        </td>
    </tr>
    
    <tr>
        <th scope="row"  class='bg-green' >대표연락처</th>
        <td colspan="3">
          <input type="text" name="mb_7" value="<?php echo $mb['mb_7'] ?>" id="mb_5" class="frm_input " size="60"  />
        </td>
    </tr>
    ...
    <!--tr>
      <th scope="row"  class='bg-green' >소재지</th>
    ...
    <tr>
        ...
        <td colspan="3">
            ...
            if (file_exists($icon_file)) {
                ...
                echo '<p><img src="'.$icon_url.'?ver='.time().'" alt="" style="max-width:500px;margin-top:5px;"></p>';
    ...
    <!--tr>
        <th scope="row" class='bg-green'>
        <?
    	if( $print_type=='partner'){?>
	
        <label for="mb_namecard">명함파일</label>
        <? }else {?>
        <label for="mb_namecard">경력증명서 또는 명함파일</label>
        <? }?>
        </th>
        <td colspan="3">
            <input type="file" name="mb_namecard" id="mb_namecard">
            <?php
            $mb_dir = substr($mb['mb_id'],0,2);
            $icon_file = G5_DATA_PATH.'/member_namecard/'.$mb_dir.'/'.$mb['mb_id'].'.gif';
            if (file_exists($icon_file)) {
                $icon_url = G5_DATA_URL.'/member_namecard/'.$mb_dir.'/'.$mb['mb_id'].'.gif';
                echo '<p><img src="'.$icon_url.'?ver='.time().'" alt="" style="max-width:500px;margin-top:5px;" ></p>';
                echo '<input type="checkbox" id="del_mb_namecard" name="del_mb_namecard" value="1">삭제';
            }
            ?>
        </td>
    </tr-->
...
<div class="btn_fixed_top">
    <a href="./<?=$return_page ? $return_page:"member_list.php"?>?<?php echo $qstr ?>" class="btn btn_02">목록</a>
...
jQuery(function($){
	/*
	$("select[name='mb_level']").on("change", function(e){
		console.log($(this).val());
		if($(this).val()=='8'){
			$("input[name='mb_1']").prop("disabled",false);
		}else{
			$("input[name='mb_1']").prop("disabled",true);
		}
