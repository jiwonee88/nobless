<?php
/**************************
@Filename: write_comment_update.sns.php
@Version : 1.0
@Author  : Freemaster
...
if ($_POST['facebook_checked']) {
    ...
    if ($user) {
        try {
            $link = G5_ADMIN_BBS_URL.'/board.php?bo_table='.$bo_table.'&wr_id='.$wr['wr_parent'].'&#c_'.$comment_id;
            $attachment = array(
...
if ($_POST['twitter_checked']) {
    ...
    if ( !(empty($_SESSION['access_token']) || empty($_SESSION['access_token']['oauth_token']) || empty($_SESSION['access_token']['oauth_token_secret'])) ) {
        $comment_url = G5_ADMIN_BBS_URL.'/board.php?bo_table='.$bo_table.'&wr_id='.$wr['wr_parent'].'&#c_'.$comment_id;
