...
if ($w == '' || $w == 'u') {
	...
	if($smb_id) {
		$sdata = sql_fetch(" select * from {$g5['cn_sub_account']} where mb_id='$mb_id' and ac_id = '$smb_id' ");
		if(!$sdata[ac_id]) alert('서브계정 정보를 찾을수 없습니다');
...
if ($w == '') {	
    ...
    $sql = " insert into {$g5['cn_reserve_table']}
                set
				mb_id = '{$mb['mb_id']}',	 
				smb_id = '{$smb_id}',	 
	...
	$return=set_insert_coin($data,$in_rsv_amt,$in_stats,1,date("Y-m-d H:i:s")."입금처리 완료\n");
    ...
    $sql = " update  {$g5['cn_reserve_table']} set 			 
	         {$sql_common}
              where in_no = '{$in_no}' ";
    sql_query($sql,1);

	//토큰/수량 변경시 취소후 재처리
	if($data['in_set_amt']!=$in_rsv_amt || $data['in_token']!=$in_token) set_insert_coin($data,$in_rsv_amt,1,1,date("Y-m-d H:i:s")."입금취소처리 완료\n");
