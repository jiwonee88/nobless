...
if ($w == "") {
	...
	*/
    ...
    if (!$is_member)
        alert('Please use after login.', G5_URL);

    //if ($member['mb_id'] != $_POST['mb_id'])
        //alert('로그인된 회원과 넘어온 정보가 서로 다릅니다.');
	$_POST['mb_id']	=$mb_id=$member['mb_id'];
...
if ($w == '') include_once('./_head.sub.php');
...
if($is_member) include_once($member_skin_path.'/register_form_update.skin.php');
else include_once($member_skin_path.'/register_form.skin.php');

if ($w == '') include_once('./_tail.sub.php');
else  include_once('./_tail.php');
?>