...
if ($mb['mb_intercept_date'] && $mb['mb_intercept_date'] <= date("Ymd", G5_SERVER_TIME)) {
    ...
    alert_json(false,'Your ID is not allowed. \nProcessed: '.$date);
}
...
//탈퇴한 회원
if ($mb['mb_10']=='2' ) {
    if ($mb['mb_leave_date'] && $mb['mb_leave_date'] <= date("Ymd", G5_SERVER_TIME)) $date = '\nWithdrawal date : '.preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})/", "\\1. \\2. \\3.", $mb['mb_leave_date']);
    alert_json(false,"You can't access this account because you've opted out.".$date);
}

//삭제한 회원
if ($mb['mb_10']=='1' ) {
    if ($mb['mb_leave_date'] && $mb['mb_leave_date'] <= date("Ymd", G5_SERVER_TIME)) $date = '\nDeletion date : '.preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})/", "\\1. \\2. \\3.", $mb['mb_leave_date']);
