...
if($w == 'u')
    ...
    alert_json(false,'The wrong approach', G5_URL);

if(!$mb_id)
    alert_json(false,'There is no member ID value. Please use the right way.');
...
if ($w == '' || $w == 'u') {
	...
	*/
    ...
    if($w=='' && $tmp_mb_name != $mb_name) {
        alert_json(false,'Please enter a valid name.');
    }
    ...
    if ($w == '' && !$mb_password)
        alert_json(false,'Password did not pass.');
    if($w == '' && $mb_password != $mb_password_re)
        alert_json(false,'Passwords do not match.');
   ...
   // if ($msg = valid_mb_nick($mb_nick))     alert($msg, "", true, true);
    ...
    if ($w=='') {
		...
		if ( get_session('ss_check_mb_id') != $mb_id ) {
            ...
            alert_json(false,'Please use the right way.');
        }
		...
		*/
        ...
        if ( $mb_recommend) {
            if (!exist_mb_id($mb_recommend))
                alert_json(false,"The  Referral Code doesn't exist.");
        }
        if (strtolower($mb_id) == strtolower($mb_recommend)) {
            alert_json(false,'You cannot enter your ID into the refererral ID');
        }		
